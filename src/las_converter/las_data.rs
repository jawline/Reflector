...
pub fn load_from_directory<F>(path: &str, max_threads: usize, filter: &LasKeepFilter, mut f: F)
where
    F: FnMut(f32, f32, f32) -> (),
{
    info!("Beginning iteration over all LAS data");
        ...
        })
    {
        ...
        pool.execute(move || {
            ...
            for wrapped_point in reader.points() {
                ...
                let is_correct_classification = filter.contains(&wrapped_point.classification);
...
impl Limits {
    pub fn load_from_directory(path: &str, max_threads: usize) -> Self {
        ...
        load_from_directory(path, max_threads, &LasKeepFilter::all(), |x, y, z| {
